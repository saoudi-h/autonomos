...
- **Workflow releases:** the published CLI pins an exact `@autonomos/core` version. Any workflow change therefore needs a patch release of both packages so `init` and `update` distribute it.
