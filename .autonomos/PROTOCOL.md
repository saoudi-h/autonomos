...
_(Crucial: User defined rules override standard defaults)_
...
3.  **Read Target AGENT.md:** If the task is in a subdirectory, check for a local `AGENT.md`.
    - _Note:_ If the local `AGENT.md` contradicts the root, the local one wins for that specific scope.
